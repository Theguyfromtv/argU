   ...
   if (err) throw err
    ...
    User.findOne({topics:{$elemMatch:{topic:newTopic,side:!newSide}}}).exec((err,user2)=>{
      ...
      if(user2){
          ...
          Chat.create({participant1id:participant1._id, participant1name:participant1.name,participant1read:false, participant2id:participant2._id, participant2name:participant2.name,participant2read:false,isActive:true,topic:newTopic}).then((err,chat)=>{
            if (err) throw err
